...
function getCurrentUser(): ?array {
    ...
    $stmt = db()->prepare("
        SELECT id, firstname, lastname, email, organisation, role, avatar_initials, created_at,
               (SELECT credential FROM integrations WHERE user_id = users.id AND service = 'whatsapp' AND is_connected = 1 LIMIT 1) as whatsapp,
               (SELECT credential FROM integrations WHERE user_id = users.id AND service = 'gmail' AND is_connected = 1 LIMIT 1) as gmail
        FROM users 
...
function addNotification(int $userId, string $title, string $message, string $type = 'info'): void {
    try {
        ...
        // 2. Récupération des intégrations actives pour cet utilisateur
        $stmtInt = db()->prepare("SELECT service, name, credential FROM integrations WHERE user_id = ? AND is_connected = 1");
        ...
        foreach ($integrations as $int) {
            ...
            if (empty($credential)) continue;

            switch ($service) {
                case 'discord':
                    // Envoi réel vers le Webhook Discord
                    $payload = json_encode([
                        'embeds' => [[
                            'title' => "🔔 NexaFlow - " . $title,
                            'description' => $message,
                            'color' => 5814783, // Couleur bleue hex décimale
                            'timestamp' => date('c')
                        ]]
                    ]);
                    $ch = curl_init($credential);
                ...
                case 'slack':
                    // Envoi réel vers le Webhook Slack
                    $payload = json_encode([
                        'text' => "🔔 *NexaFlow - {$title}*\n{$message}"
                    ]);
                    $ch = curl_init($credential);
                ...
                case 'whatsapp':
                    // Simulation d'envoi WhatsApp dans un fichier log local
                    $dir = __DIR__ . '/../uploads';
                    if (!is_dir($dir)) @mkdir($dir, 0777, true);
                    $logFile = $dir . '/whatsapp_simulations.log';
                    $logMsg = "[" . date('Y-m-d H:i:s') . "] WhatsApp ($credential) : *{$title}* - {$message}\n";
                    @file_put_contents($logFile, $logMsg, FILE_APPEND);
                    break;

                case 'gmail':
                    // Envoi réel par mail + journalisation
                    $to = $credential;
                    $subject = "NexaFlow - " . $title;
                    $headers = "From: no-reply@nexaflow.com\r\nReply-To: no-reply@nexaflow.com\r\nContent-type: text/html; charset=UTF-8\r\n";
                    $htmlMessage = "<html><body style='font-family:sans-serif;color:#1e293b;'>";
                    $htmlMessage .= "<div style='padding:20px;background:#f8fafc;border-radius:10px;'>";
                    $htmlMessage .= "<h2 style='color:#3b82f6;'>🔔 {$title}</h2>";
                    $htmlMessage .= "<p style='font-size:15px;line-height:1.5;'>{$message}</p>";
                    $htmlMessage .= "<hr style='border-color:#e2e8f0;'><p style='font-size:12px;color:#64748b;'>Cet email automatique provient de votre espace NexaFlow.</p>";
                    $htmlMessage .= "</div></body></html>";
                    @mail($to, $subject, $htmlMessage, $headers);

                    $dir = __DIR__ . '/../uploads';
                    if (!is_dir($dir)) @mkdir($dir, 0777, true);
                    $logFile = $dir . '/email_simulations.log';
                    $logMsg = "[" . date('Y-m-d H:i:s') . "] Gmail ($to) : {$title} - {$message}\n";
                    @file_put_contents($logFile, $logMsg, FILE_APPEND);
                    break;

                default:
                    // Journalisation pour les autres intégrations (Zoom, GitHub, Figma, etc.)
                    $dir = __DIR__ . '/../uploads';
                    if (!is_dir($dir)) @mkdir($dir, 0777, true);
                    $logFile = $dir . '/integrations_simulations.log';
                    $logMsg = "[" . date('Y-m-d H:i:s') . "] Intégration [{$service}] ({$credential}) : {$title} - {$message}\n";
                    @file_put_contents($logFile, $logMsg, FILE_APPEND);
                    break;
            }
        }
