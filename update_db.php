...
try {
    ...
    // Table: users - add whatsapp and gmail columns if they don't exist
    try {
        $pdo->exec("ALTER TABLE users ADD COLUMN whatsapp VARCHAR(50) DEFAULT NULL");
        echo "- Colonne 'whatsapp' ajoutée à la table 'users'.<br>";
    } catch (PDOException $e) {
        // Ignorer si la colonne existe déjà
    }
    try {
        $pdo->exec("ALTER TABLE users ADD COLUMN gmail VARCHAR(255) DEFAULT NULL");
        echo "- Colonne 'gmail' ajoutée à la table 'users'.<br>";
    } catch (PDOException $e) {
        // Ignorer si la colonne existe déjà
    }
