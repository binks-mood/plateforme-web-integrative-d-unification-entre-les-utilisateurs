    ...
    if (data.recent_projects.length === 0) {
      ...
      projList.innerHTML = data.recent_projects.map(p => `
        <div class="project-item" style="display:flex;align-items:center;gap:12px;padding:12px;border-radius:10px;margin-bottom:8px;background:rgba(59,130,246,.05);position:relative;cursor:pointer;flex-direction:column;align-items:stretch" onclick="showPage('projects')">
          ...
          ${(NX_USER.role === 'admin' || NX_USER.email === 'emmanuel.soonet@admin.com') ? `
...
function openUserProfile(id) {
    ...
    document.getElementById('upRole').textContent = user.role_label;
...
async function loadIntegrations() {
  try {
    ...
    grid.innerHTML = res.data.map(int => `
      <div class="col-12 col-sm-6 col-md-4 col-xl-3">
      <div class="integration-card ${int.is_connected ? 'connected' : ''}" onclick="toggleIntegration('${int.service}','${escHtml(int.name)}')">
        <div class="integration-logo">${int.icon}</div>
        <div class="integration-name">${escHtml(int.name)}</div>
        <div class="integration-desc">${escHtml(int.desc)}</div>
        <button class="btn btn-sm ${int.is_connected ? 'btn-secondary' : 'btn-primary'}" style="width:100%">
          ${int.is_connected ? '✅ Connecté' : 'Connecter'}
        </button>
      </div>
      </div>
    `).join('');
...
async function loadAnalytics() {
  try {
    ...
    document.getElementById('analyticsStats').innerHTML = `
      <div class="col-12 col-sm-6 col-lg-3"><div class="stat-card"><div class="stat-icon blue">📈</div><div class="stat-value">87%</div><div class="stat-label">Taux de complétion</div><div class="stat-change up">↑ 5% ce mois</div></div></div>
      <div class="col-12 col-sm-6 col-lg-3"><div class="stat-card"><div class="stat-icon green">⚡</div><div class="stat-value">3.2j</div><div class="stat-label">Temps moyen/tâche</div><div class="stat-change up">↑ Amélioration</div></div></div>
      <div class="col-12 col-sm-6 col-lg-3"><div class="stat-card"><div class="stat-icon yellow">🎯</div><div class="stat-value">94%</div><div class="stat-label">Dans les délais</div><div class="stat-change up">↑ 2%</div></div></div>
      <div class="col-12 col-sm-6 col-lg-3"><div class="stat-card"><div class="stat-icon purple">💬</div><div class="stat-value">456</div><div class="stat-label">Messages échangés</div><div class="stat-change up">↑ Actif</div></div></div>
    `;
...
function handleSearch(val) {
  ...
  // Filtrer les tâches visibles si on est sur la page tâches
  if (currentPage === 'tasks' && allTasks.length) {
    const filtered = allTasks.filter(t =>
      t.title.toLowerCase().includes(val.toLowerCase()) ||
      (t.project_name||'').toLowerCase().includes(val.toLowerCase())
    );
...
function viewSearchResult(page, id) {
  ...
  showPage(page);
  // Optionnel: on pourrait scroller vers l'item ou ouvrir un détail
}
...
document.addEventListener('DOMContentLoaded', async () => {
  await refreshSidebarBadges(); // Attendre que les stats soient chargées avant d'afficher la page initiale
