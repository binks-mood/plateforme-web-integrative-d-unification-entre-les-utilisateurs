...
// ── Analytics ──────────────────────────────────────────
// Tâches avec échéance respectée (done et updated_at <= due_date)
$s = db()->prepare("SELECT COUNT(*) as c FROM tasks WHERE status = 'done' AND due_date IS NOT NULL AND DATE(updated_at) <= due_date $task_filter");
$s->execute(); $out['tasks_on_time'] = (int)$s->fetch()['c'];

// Tâches avec échéance définie
$s = db()->prepare("SELECT COUNT(*) as c FROM tasks WHERE due_date IS NOT NULL $task_filter");
$s->execute(); $out['tasks_with_due'] = (int)$s->fetch()['c'];

// Durée moyenne de complétion en jours
$s = db()->prepare("SELECT AVG(DATEDIFF(updated_at, created_at)) as avg_days FROM tasks WHERE status = 'done' $task_filter");
$s->execute(); $out['tasks_avg_days'] = round((float)($s->fetch()['avg_days'] ?? 0), 1);

// Tâches dues dans les 48h (alerte deadline)
$s = db()->prepare("SELECT t.id, t.title, t.due_date, t.priority, p.name as project_name FROM tasks t LEFT JOIN projects p ON t.project_id = p.id WHERE t.status != 'done' AND t.due_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 2 DAY) $task_filter ORDER BY t.due_date ASC LIMIT 10");
$s->execute(); $out['tasks_due_soon'] = $s->fetchAll();
