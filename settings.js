    ...
    profile: `
      <div style="display:grid;grid-template-columns:1fr 2fr;gap:28px">
        <div class="card" style="text-align:center">
          <div id="avatarPreviewWrap" style="margin:0 auto 16px;width:80px;height:80px;position:relative;cursor:pointer" onclick="document.getElementById('avatarFileInput').click()">
            ${u.avatar_url
              ? `<img src="${u.avatar_url}" alt="Avatar" style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:2px solid var(--blue-600)">`
              : `<div class="avatar avatar-xl" style="width:80px;height:80px;font-size:28px">${initials}</div>`}
            <div style="position:absolute;bottom:0;right:0;background:var(--blue-600);border-radius:50%;width:24px;height:24px;display:flex;align-items:center;justify-content:center;font-size:12px">📷</div>
          </div>
          ...
          <button class="btn btn-secondary btn-sm" style="width:100%" onclick="document.getElementById('avatarFileInput').click()">📷 Changer la photo</button>
        </div>
        <div class="card">
          ...
          <div class="form-row">
            <div class="form-group"><label>Numéro WhatsApp (Géré via les intégrations)</label><div class="input-wrapper"><input type="text" id="settWhatsapp" value="${u.whatsapp || ''}" placeholder="Non configuré (allez dans l'onglet Intégrations)" style="padding-left:14px; background:rgba(255,255,255,0.05); cursor:not-allowed;" readonly/></div></div>
            <div class="form-group"><label>Adresse Gmail (Gérée via les intégrations)</label><div class="input-wrapper"><input type="email" id="settGmail" value="${u.gmail || ''}" placeholder="Non configurée (allez dans l'onglet Intégrations)" style="padding-left:14px; background:rgba(255,255,255,0.05); cursor:not-allowed;" readonly/></div></div>
          </div>
...
async function saveProfile() {
  ...
  if (res.success) {
    ...
    NX_USER.firstname = firstname;
    NX_USER.lastname = lastname;
    NX_USER.organisation = organisation;
    // Update display name if elements exist
...
async function uploadAvatar(input) {
  ...
  try {
    const res = await fetch('api/team/upload_avatar.php', { method: 'POST', body: formData });
    const json = await res.json();
    if (json.success) {
      showToast(json.message, 'success');
      NX_USER.avatar_url = json.avatar_url;
      // Rafraîchir la préview immédiatement
      const wrap = document.getElementById('avatarPreviewWrap');
      if (wrap) {
        const img = wrap.querySelector('img') || document.createElement('img');
        img.src = json.avatar_url + '?t=' + Date.now();
        img.style.cssText = 'width:80px;height:80px;border-radius:50%;object-fit:cover;border:2px solid var(--blue-600)';
        const avatar = wrap.querySelector('.avatar');
        if (avatar) wrap.replaceChild(img, avatar);
