...
<body>
<script>if (localStorage.getItem('nx-theme') === 'light') document.body.classList.add('light-mode');</script>
...
<div class="app-layout">
  ...
  <nav class="sidebar" id="sidebar">
    <div class="sidebar-header">
      ...
      <div class="sidebar-header-actions">
        ...
        <button class="header-theme-btn" id="headerThemeBtn" onclick="toggleTheme()" title="Changer le thème" aria-label="Changer le thème">
    ...
    <div class="sidebar-nav">
      ...
      <div class="nav-section">
        ...
        <?php endif; ?>
        <div class="nav-item <?= $user['role'] !== 'admin' ? 'active' : '' ?>" onclick="showPage('projects')" id="nav-projects">
  ...
  <div class="main-content">
    ...
    <main class="page-content" id="pageContent">
      ...
      <!-- Barre alerte deadline -->
      <div id="deadlineAlertBar" style="display:none;align-items:center;gap:12px;background:linear-gradient(90deg,#b45309,#d97706);color:white;padding:10px 20px;border-radius:10px;margin-bottom:16px;font-size:13px"></div>
      ...
      <!-- ══ PAGE: PROJETS ════════════════════════════ -->
      <section class="page-section <?= $user['role'] !== 'admin' ? 'active' : '' ?>" id="page-projects">
        <div class="section-header">
          ...
          <div style="display:flex;gap:8px">
            <button class="btn btn-secondary btn-sm" style="width:auto" onclick="exportCSV('projects')" title="Exporter en CSV">📥 Export CSV</button>
      ...
      <section class="page-section" id="page-tasks">
        <div class="section-header">
          ...
          <div style="display:flex;gap:8px">
            <button class="btn btn-secondary btn-sm" style="width:auto" onclick="exportCSV('tasks')" title="Exporter en CSV">📥 Export CSV</button>
      ...
      <section class="page-section" id="page-timeline">
        ...
        <div class="card">
          <div style="overflow-x:auto">
            <div id="ganttChart" style="width:100%;padding-top:8px"></div>
          </div>
        </div>
      </section>
      ...
      <section class="page-section" id="page-integrations">
        ...
        <div style="display:flex;gap:10px;margin-bottom:24px;flex-wrap:wrap">
          <button class="btn btn-primary btn-sm integration-filter-btn" id="filter-all" style="width:auto" onclick="filterIntegrations('all')">Tous</button>
          <button class="btn btn-secondary btn-sm integration-filter-btn" id="filter-communication" onclick="filterIntegrations('communication')">Communication</button>
          <button class="btn btn-secondary btn-sm integration-filter-btn" id="filter-developpement" onclick="filterIntegrations('developpement')">Développement</button>
          <button class="btn btn-secondary btn-sm integration-filter-btn" id="filter-stockage" onclick="filterIntegrations('stockage')">Stockage</button>
          <button class="btn btn-secondary btn-sm integration-filter-btn" id="filter-analytics" onclick="filterIntegrations('analytics')">Analytics</button>
          <button class="btn btn-secondary btn-sm integration-filter-btn" id="filter-design" onclick="filterIntegrations('design')">Design</button>
          <button class="btn btn-secondary btn-sm integration-filter-btn" id="filter-gestion" onclick="filterIntegrations('gestion')">Gestion</button>
          <button class="btn btn-secondary btn-sm integration-filter-btn" id="filter-documentation" onclick="filterIntegrations('documentation')">Documentation</button>
          <button class="btn btn-secondary btn-sm integration-filter-btn" id="filter-cloud" onclick="filterIntegrations('cloud')">Cloud</button>
        </div>
...
<div class="modal-overlay" id="userProfileModal">
  <div class="modal-box" style="max-width: 400px;">
    ...
    <div id="userProfileContent" style="text-align:center; padding: 10px 0;">
      ...
      <p style="margin-bottom:16px"><span class="badge badge-blue" id="upRole">Développeur</span></p>
      
      <div style="text-align:left; margin-bottom: 20px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); padding:16px; border-radius:12px; font-size:13px;">
        <div style="margin-bottom:8px;">🏢 <strong>Organisation :</strong> <span id="upOrg">—</span></div>
        <div style="margin-bottom:8px;" id="upWhatsappRow">💬 <strong>WhatsApp :</strong> <span id="upWhatsapp">—</span></div>
        <div style="margin-bottom:0;" id="upGmailRow">📧 <strong>Gmail :</strong> <span id="upGmail">—</span></div>
      </div>
      
      <?php if ($user['role'] === 'admin'): ?>
      <div id="upTeamSection" style="text-align:left; background:rgba(59,130,246,.08); border:1px solid rgba(59,130,246,.2); padding:16px; border-radius:12px;">
...
<div class="modal-overlay" id="connectIntegrationModal">
  <div class="modal-box" style="max-width:480px">
    <div class="modal-header">
      <span class="modal-title" id="connectModalTitle">🔗 Connecter</span>
      <button class="modal-close" onclick="closeModal('connectIntegrationModal')">✕</button>
    </div>
    <input type="hidden" id="connectModalService">
    <input type="hidden" id="connectModalName">
    <div class="form-group">
      <label id="connectCredLabel">Identifiant</label>
      <div class="input-wrapper">
        <input type="text" id="connectCredInput" placeholder="" style="padding-left:14px" onkeydown="if(event.key==='Enter')submitConnectIntegration()"/>
      </div>
      <p id="connectModalHelp" style="font-size:12px;color:var(--gray-500);margin-top:8px;line-height:1.6"></p>
    </div>
    <div class="modal-footer">
      <button class="btn btn-ghost" onclick="closeModal('connectIntegrationModal')">Annuler</button>
      <button class="btn btn-primary" id="connectModalSubmitBtn" style="width:auto;padding:10px 24px" onclick="submitConnectIntegration()">Connecter</button>
    </div>
  </div>
</div>
...
<div class="modal-overlay" id="taskCommentsModal">
  <div class="modal" style="max-width:560px">
    <div class="modal-header">
      <span class="modal-title">💬 <span id="taskCommentTitle">Commentaires</span></span>
      <button class="modal-close" onclick="closeModal('taskCommentsModal')">✕</button>
    </div>
    <div id="commentsContainer" style="max-height:340px;overflow-y:auto;padding:4px 0;margin-bottom:16px"></div>
    <div style="display:flex;gap:8px;align-items:flex-end">
      <textarea id="commentInput" placeholder="Écrire un commentaire… (Entrée pour envoyer)" onkeydown="handleCommentKey(event)"
        style="flex:1;resize:none;height:60px;padding:10px 14px;background:rgba(10,22,40,.6);border:1px solid rgba(59,130,246,.2);border-radius:10px;color:white;font-size:13px;outline:none;font-family:inherit"></textarea>
      <button class="btn btn-primary" style="width:auto;padding:10px 16px;height:60px" onclick="submitComment()">Envoyer</button>
    </div>
  </div>
</div>
</body>
